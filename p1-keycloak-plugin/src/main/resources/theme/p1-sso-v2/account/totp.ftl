  ...
  <table class="table table-bordered table-striped">
    <thead>
      <#if totp.otpCredentials?size gt 1>
        <tr>
          <th colspan="4" class="unheader"><h2>${msg("configureAuthenticators")}</h2></th>
        </tr>
      <#else>
        <tr>
          <th colspan="3" class="unheader"><h2>${msg("configureAuthenticators")}</h2></th>
        </tr>
      </#if>
    </thead>
    <tbody>
      <#list totp.otpCredentials as credential>
        <tr>
          ...
          <td class="action">
            <form action="${url.totpUrl}" method="post" class="form-inline text-center">
              <input type="hidden" id="stateChecker" name="stateChecker" value="${stateChecker}">
              <input type="hidden" id="submitAction" name="submitAction" value="Delete">
              <input type="hidden" id="credentialId" name="credentialId" value="${credential.id}">
              <button id="remove-mobile" class="btn btn-error" data-toggle="tooltip" data-placement="right" title="Remove Authenticator">
                Revoke access
              </button>
            </form>
          </td>
